...
*/
static __inline UInt8 linear_to_ulaw(int linear) {
  UInt8 u_val;
  int mask;
  int seg;
   ...
   */
  if (seg >= 8)
    u_val = (UInt8)(0x7F ^ mask);
  else
    u_val = (UInt8)(((seg << 4) | ((linear >> (seg + 3)) & 0xF)) ^ mask);
#ifdef ULAW_ZEROTRAP
  /* Optional ITU trap */
  if (u_val == 0)
    u_val = 0x02;
...
*/
static __inline Int16 ulaw_to_linear(UInt8 ulaw) {
  int t;
  ...
  return (Int16)((ulaw & 0x80) ? (ULAW_BIAS - t) : (t - ULAW_BIAS));
}
...
*/
static __inline UInt8 linear_to_alaw(int linear) {
  int mask;
  int seg;
  ...
  if (seg >= 8) {
    ...
    return (UInt8)(0x00 ^ mask);
  }
  /* Combine the sign, segment, and quantization bits. */
  return (UInt8)(((seg << 4) | ((linear >> ((seg) ? (seg + 3) : 4)) & 0x0F)) ^
                   mask);
}
...
/*! \brief Decode an A-law sample to a linear value.
    ...
    \return The linear value.
*/
static __inline Int16 alaw_to_linear(UInt8 alaw) {
  int i;
  int seg;
...
*/
UInt8 alaw_to_ulaw(UInt8 alaw);

/*! \brief Transcode from u-law to A-law, using the procedure defined in G.711.
    \param alaw The u-law sample to transcode.
    \return The best matching A-law value.
*/
UInt8 ulaw_to_alaw(UInt8 ulaw);
